...
export function DealsAdmin({ initialDeals, menuItems, categories }: { initialDeals: Deal[]; menuItems: MenuItem[]; categories: Category[] }) {
  ...
  const openEdit = (deal: Deal) => {
    ...
    // Initialize tabs based on data, using the DB slots order (which maps to index for now)
    // When fields map runs, field.id is generated by react-hook-form, so we can't easily pre-map field.id here.
    // Instead we will let the Tabs fallback to watch() initially, and only override when clicked.
    setActiveTabs({});
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-12 gap-4">
                            ...
                            <div className="sm:col-span-12 bg-muted/40 p-3 border border-black/5 space-y-3">
                              ...
                              <Tabs 
                                ...
                                onValueChange={(val) => {
                                  setActiveTabs(prev => ({ ...prev, [field.id]: val }));
                                  if (val === "text") {
