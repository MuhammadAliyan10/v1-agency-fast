// components/features/admin/finance/register-date-filter.tsx
"use client";
...
interface RegisterDateFilterProps {
  from?: string;
  to?: string;
}
...
export function RegisterDateFilter({ from, to }: RegisterDateFilterProps) {
  ...
  const applyPreset = (preset: PresetKey) => {
    ...
    if (preset === "week") {
      f = format(subDays(now, 6), "yyyy-MM-dd");
    }
  ...
  return (
    <div className="flex items-center gap-1.5 bg-muted/40 border border-border/60 px-2 py-1.5 flex-wrap">
      {PRESETS.map(p => (
        <Button
          key={p.key}
          variant="ghost"
